...
Stage 1 (M1–M11) + Stages 1.5–1.10 + 1.99 + Stage 2 (G-M1–M17, except v2 voice biometrics) + Stage 3 (security) + Stage 4 (desktop M2 premium recorder) + Stage 5 (desktop M3 visual restructure) + Stage 6 (live notes) + Stage 7 (stability + Granola UX + multi-folder Phase 1) + Stage 8 (Granola-grade desktop note workspace) + Stage 9 (reliability sprint: orphan recovery + boot-warmed pg-boss + brownout detection) + Stage 10 (open-source readiness: compose+MinIO, doctor, first-run+invites+reset, failure UX+watchdog+retry, real /api/health, boot-warm attempt 2, whisper provider, configurable extension 0.9.0, release workflows, v1.0.0) all shipped. Big-picture surface area:
