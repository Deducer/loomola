...
- **`src/instrumentation.ts`** — Next.js App Router boot hook calls `getBoss()` on container start so workers begin polling immediately. Without this, after a Coolify restart pg-boss is dead until something fires an `enqueueX` call. Recordings used to silently sit in `transcribing` for hours.
