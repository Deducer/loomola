...
## Stage 4 — Desktop M2 (in progress, premium recorder)
...
| Phase 1 — Composite writer | 🚧 partial | All inputs wired: `ScreenCaptureCoordinator.onScreenSampleBuffer` callback + `latestScreenPixelBuffer()`, new `CameraCaptureCoordinator` (single-source-of-truth shared session, fed to bubble overlay AND future compositor), `MicrophoneCaptureCoordinator` rewritten on AVAudioEngine + `setVoiceProcessingEnabled` (AEC) + `onSampleBuffer` callback. Real `CompositeRecorder` built (AVAssetWriter + CIContext, screen + bubble compose with circle/rectangle alpha mask, AAC mic input). **Not yet wired into `RecorderViewModel`** — production recording still uses the M1 SCRecordingOutput path. The compositor compiles + has the right interface; end-to-end smoke comes when the wiring lands. |
| Phase 2 — Recording HUD | pending | |
| Phase 3 — Source picker | pending | |
| Phase 4 — Permissions preflight | pending | |
| Phase 5 — Menubar quick-record + global hotkey | partial | Menubar toggle (Show/Hide bubble) shipped. Real `RegisterEventHotKey` global hotkey deferred. |
| Phase 6 — E2E smoke + docs | pending | |

**Bubble polish along the way (all shipped):** menubar Show/Hide toggle with `NSMenuItemValidation` updating the title; bubble overlay subscribes to `BubblePositionController` and publishes a fresh placement on every drag tick; bubble panel marked `sharingType = .none` so the future compositor's screen capture is "naked" of the overlay (it composites the bubble independently); custom drag handler instead of `isMovableByWindowBackground` to bypass AppKit's window-drag manager; `.hudWindow + .popUpMenu + .transient + .stationary + .ignoresCycle` to opt out of Stage Manager / Mission Control / window cycling; scroll-wheel to resize the bubble (90–360 pt, ⌥/⇧ for slow); dark `black @ 0.32` placeholder background instead of jarring purple flash on hide.
...
| **Wire `CompositeRecorder` into `RecorderViewModel`** (M2 Phase 1 completion) | The compositor exists + is callable but not yet consumed by the recording flow. Wiring is the smoke test. | ~3-4 hours |
