...
**Pending / out-of-scope deferrals:** the three floating panel windows (`AudioRecordingWindowController`, `VideoRecordingWindowController`, `MeetingPromptWindowController`) still use M2-era inline styling — small follow-up to retoken with `DSColor` / `DSFont` (~30 min each). User E2E smoke pending dogfood session.
...
|---|---|---|
| **Desktop M3 floating-panel retoken** | The three floating panel windows (audio recording HUD, video recording HUD, meeting prompt) still use M2-era inline styling. Retoken with DSColor / DSFont / DSRadius / DSShadow so they read as the same product as the new shell. | ~1 hour total |
