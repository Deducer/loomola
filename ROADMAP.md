...
**Deferred from the same audit (next up):** merged single-call AI outputs (title+summary+chapters+action-items in one Claude call, ~57% LLM cost cut), video orphan recovery on desktop + web (biggest data-loss gap), desktop server-URL onboarding for self-hosters, live-transcription reconnect backoff, AI Q&A on a note.
...
|---|---|---|
| **Live-transcription reconnect with backoff** | The Deepgram live WebSocket dies permanently on any network churn — VPN toggle (Tailscale profile switch for Credit Builder Card admin access), wifi handoff, sleep/wake. `LiveTranscriptionCoordinator.apply(source:state:)` logs `.failed` and gives up. Fix: on `.failed`/unexpected `.closed` while recording, mint a fresh `/api/transcribe/live-token`, reopen the socket with exponential backoff (cap ~30s), and mark a gap in the transcript UI. Deferred from the Stage 11 audit; Stage 12's Tailscale investigation confirmed it's the last recording-adjacent casualty of mid-call VPN churn. | ~half day |
