...
| **Real Today pill (Phase A)** | `media_objects.calendar_event_title/started_at` (migration 0029) persist which event the desktop matched at recording start; the workspace Today pill shows it. Popover: event provenance, "Wrong event? Pick another" and "Link an event from today" — linking re-resolves attendees via /api/people/resolve, PATCHes them with the event, and re-runs speaker suggestions. Critical for the no-calendar-event case (verified real: the 2026-07-07 Ann Callison call had no event; attendees were added manually). |
...
|---|---|---|
| **Transcript-content speaker attribution** | The G-M13 matcher maps non-self speakers to attendees POSITIONALLY — correct only by luck for 3+ person calls, and unusable when invited ≠ spoke. The real evidence is in the transcript: people address each other by name ("Thanks, Ann" → the responding voice is Ann; "Bhaskar, what do you think?" → next speaker is Bhaskar). An LLM pass over the diarized transcript + attendee list could produce evidence-based per-speaker attributions with confidence, replacing positional mapping. Bridges most of the gap to voice biometrics (Path C) at ~zero infra cost. | ~1–2 days |
