...
| **Calendar-aware pre-meeting prompt** | Today the desktop only fires "Meeting ready" once a Meet/Zoom/Teams *window* is detected. Granola-style behavior also fires N minutes before a *scheduled* event. Adds EventKit integration + permission flow + 1-minute-before notification with the same Start note / Open meeting actions. Calendar event description / location is parsed for Meet/Zoom/Teams URLs so the join button works pre-meeting. **Constraint:** never fire reminders *after* the meeting has started — see [memory feedback](../../../.claude/projects/-Users-iancross-Development-03Utilities-Loom-Clone/memory/feedback_no_persistent_meeting_reminders.md). | ~half day |
| **On-screen content sanitization (privacy redaction)** | Live-blur or block-out sensitive on-screen content during recording — emails, API keys, customer names, credit cards. Two-mode design: (a) regex-based auto-detection for common patterns (email, JWT, AWS keys, etc.) with toggleable categories, (b) user-defined regions / windows masked with a black rectangle in the composite output. The compositor (already CIContext-based) is the right place to insert a CIFilter pass per frame. Defer until M3 has bedded in. | ~3–4 days for MVP |
