...
| **Floating cross-Spaces recording pill** | ✅ shipped | Granola-shape vertical capsule (~36×88pt) shown for the duration of an audio note recording. Floats on every Space and every app (`canJoinAllSpaces + .stationary + .fullScreenAuxiliary`) so the user is reminded even when they're in Zoom, Slack, Chrome, or on a different desktop. Loomola brand mark + 3-bar live meter; hover reveals the latest live transcript with confirmed vs dim in-progress text; tap brings Loomola to the front + opens workspace; position persists in UserDefaults. `sharingType: .none` keeps both pill and transcript preview out of the user's own captures. Spec: [`2026-05-06-floating-recording-pill-design.md`](docs/superpowers/specs/2026-05-06-floating-recording-pill-design.md). |
| **Live transcription** | ✅ shipped | Deepgram streaming runs alongside capture, with confirmed and interim text in the workspace transcript drawer. The floating recording pill now exposes the same stream on hover so transcription health is visible without leaving Zoom or another meeting app. |
...
Known limitations left deliberate: calendar events with zero invited attendees can now preserve event provenance but provide no People candidates for speaker attribution (the ad-hoc 1:1 gap; see the follow-up below); re-linking never overwrites accepted/dismissed names; batch diarization over-segments group calls (a 6-person meeting produced 12 voice indices), so strict count-matching rarely fires there — self-detection + the Stage-17 LLM pass carry those meetings.

## Open follow-ups (next milestones to spec)

| Topic | Why | Rough effort |
|---|---|---|
| **Mic survives a device change mid-recording** | `MicrophoneCaptureCoordinator.scheduleEngineRestart` deliberately suppresses engine restarts during file-backed audio notes, so an input switch mid-call (AirPods connecting, Zoom grabbing the mic, sample-rate change) silences the mic for the rest of the note. Since 2026-09-22 the write-health alert at least notifies the user, but audio is still lost. Fix: restart the engine into the same `AudioAssetWriter` (it already resamples any input rate to 48 kHz). Found while fixing the Bluetooth 24 kHz encoder failure (commit 171d2ad). | ~half day + real-device test |
...
| **Calendar-aware pre-meeting prompt** | **Installed 2026-07-27; delivery/action observation pending.** EventKit schedules macOS notifications one minute before joinable Meet/Zoom/Teams/Webex/FaceTime events, with Join and Take Notes actions; it never creates a reminder after start. Live verification confirmed notification permission, two real scheduled reminders, Settings diagnostics, Launch at Login, and a meeting-only Today picker. The same reliability pass added low-cost native Zoom-window polling and immediate active-note calendar hydration. The banner/actions and a positive native Zoom match remain to be observed during the next eligible event/call. Spec: [`2026-07-27-calendar-reminder-reliability-design.md`](docs/superpowers/specs/2026-07-27-calendar-reminder-reliability-design.md). | Observe next real reminder + active Zoom match |
